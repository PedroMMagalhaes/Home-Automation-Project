...
#define humidity_topic "sensor/raindrops"        //Topic raindrops
...
void loop() {
  ...
  if (now - lastMsg > 1000 * 5) {
    ...
    client.publish(humidity_topic, String(rain).c_str(), true);      // Publish rainstatus (its not necessary conversion to string , but in case of spaces is required)
  }
  if (now - lastRecu > 100 ) {
    lastRecu = now;
